...
import { collection, addDoc, getDoc, doc as firestoreDoc, serverTimestamp } from 'firebase/firestore';
...
export default {
  ...
  computed: {
    ...
    frecuenciaMeses() {
      const map = { Mensual: 1, Bimestral: 2, Trimestral: 3, Cuatrimestral: 4, Semestral: 6, Anual: 12 };
  ...
  async mounted() {
    this.cargaId = this.$route.query.cargaId;
    if (this.cargaId) {
      await this.cargarCronogramaDesdeFirestore();
    } else {
      this.generarCronograma();
    }
  },
  methods: {
    async cargarCronogramaDesdeFirestore() {
      try {
        const snap = await getDoc(firestoreDoc(db, "cronogramas", this.cargaId));
        if (snap.exists()) {
          this.cronograma = snap.data().cronograma;
        }
      } catch (e) {
    ...
    logout() {
      const auth = getAuth();
      signOut(auth).then(() => this.$router.push({ name: 'login' })).catch(console.error);
    },
    descargarYGuardar() {
      this.exportarExcel();
